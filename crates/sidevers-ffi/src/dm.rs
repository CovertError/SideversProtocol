...
pub unsafe extern "C" fn sv_dm_seal_text(
    ...
    // SAFETY: caller contract.
    let recipient_pk = unsafe { std::slice::from_raw_parts(recipient_pubkey_32, PUBLIC_KEY_LEN) };
    ...
    let result = (|| -> sidevers_core::Result<Vec<u8>> {
        ...
        let ciphertext = core_payload::seal(&inner_payload, &side, &recipient_arr, &nonce, b"")?;
...
pub unsafe extern "C" fn sv_dm_open_text(
    ...
    let plain = match core_payload::open(&env.payload, &recipient_side, &env.from, &env.nonce, b"")
    {
        Ok(p) => p,
        Err(e) => {
            let (status, _) = status_from::<()>(Err(e));
